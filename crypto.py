...
class Crypto(object):
    ...
    def generate_keys():
        ...
        if not os.path.isfile(path_public_key) and not os.path.isfile(path_private_key):
            ...
            print(colored(f"Ключи шифрования созданы: {public_file} и {private_file}", 'green'))
            print('Необходимо указать их расположение в файле *.ini')
            print(colored(f"Файл {private_file} должен находиться в зашифрованном хранилище", 'white', 'on_red'))

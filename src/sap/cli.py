...
def sap_systems_list_into_nametuple(result: list) -> Sap_system:
    ...
    passwords = [
        None
        if isinstance(item[3], type(None))
        else (Crypto.decrypto(item[3]) if isinstance(item[3], bytes) else item[3])
        for item in result
    ]
...
def start():
    ...
    click.echo(
        click.style(
            f"Ключи шифрования созданы: {Crypto.public_file} и {Crypto.private_file}",
            **utilities.color_success,
        )
    )
    ...
    click.echo(
        click.style(
            f"Файл {Crypto.private_file} должен находиться в зашифрованном хранилище",
            **utilities.color_sensitive,
        )
    )
